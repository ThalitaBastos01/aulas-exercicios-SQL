create database exercicio_crud_sql; -- nome 
--1 criação da tabela 
...
--4 excluindo os dados duplicados
delete from usuarios where id = 5; -- caso eu queira excluir somente 1 dos dados duplicado
delete from usuarios where email = 'raja.feugiat@nonummy.com'; --caso eu queira excluir os dois dados duplicado


alter table usuarios add constraint email_unique_usuarios unique(email);


--5 inserindo mais dados e adicionando uma nova coluna
...
-- criou a coluna situação na tabela
alter table usuarios add column situacao boolean default true;

--6 adicionando situação do usuario
...
-- excluindo o campo idade e adicionando data de nascimento

alter table usuarios drop column idade, add column data_nascimento date;

--8 atualizando a data de nascimento 
...
-- 9 excluir todos os registros que não tem data de nascimento 
...
-- 10 adicionando dados atualizados 
